...
export const useFileStore = create<State>((set, get) => ({
  ...
  open: async (path) => {
    ...
    try {
      set({
        entries: (await api.list(path)).entries,
        loading: false,
        busy: null,
      });
    } catch (error) {
  ...
  upload: async (files) => {
    ...
    const tasks = list.map((file, index) => ({
      id: `${Date.now()}-${index}`,
    ...
    for (const [index, file] of list.entries()) {
      ...
      set((state) => ({
        uploadTasks: state.uploadTasks.map((item) =>
      ...
      try {
        ...
        const remote =
          get().path === "." ? relative : `${get().path}/${relative}`;
        await api.uploadFile(file, remote, (progress) =>
          set((state) => ({
            uploadTasks: state.uploadTasks.map((item) =>
  ...
  remove: async (entry) => {
    ...
    try {
      await api.remove(entry.path, entry.type === "directory");
      await get().open(get().path);
    } catch (error) {
  ...
  moveEntry: async (entry, directory) => {
    ...
    const target =
      cleanDirectory && cleanDirectory !== "."
        ? `${cleanDirectory}/${entry.name}`
        : entry.name;
    set({ busy: "move", error: null });
    try {
      await api.move(entry.path, target);
      await get().open(get().path);
    } catch (error) {

...
async function request<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!value.ok)
    throw new Error(
      `${value.error?.code ?? "request_failed"}: ${value.error?.message ?? "request failed"}`,
    );
  return value;
}

export const list = (path: string) =>
  request<{ entries: Entry[] }>(`/api/list?path=${encodeURIComponent(path)}`);
export const stat = (path: string) =>
  request<{ entry: Entry }>(`/api/stat?path=${encodeURIComponent(path)}`);
export const mkdir = (path: string) =>
  request(`/api/mkdir?path=${encodeURIComponent(path)}`, { method: "POST" });
export const move = (from: string, to: string, overwrite = false) =>
  request("/api/move", {
    method: "POST",
...
export const copy = (
  ...
  request("/api/copy", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ from, to, overwrite, recursive }),
  });
...
export async function uploadFile(
  ...
  const begin = await request<{ uploadId: string; nextOffset: number }>(
    "/api/upload/begin",
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  while (offset < file.size) {
    ...
    const result = await request<{ nextOffset: number }>(
      `/api/upload/chunk/${begin.uploadId}/${offset}/${chunkSha}`,
...
export async function fetchFileBlob(
  ...
  for (let offset = 0; offset < info.size; offset += chunkSize) {
    const response = await fetch(
      `/api/download/chunk?path=${encodeURIComponent(path)}&offset=${offset}&size=${chunkSize}`,
    );
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
